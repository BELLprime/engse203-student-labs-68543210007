# API Contract — ณัฏฐกิตติ์ รอดเรือน
...
| **ผู้จัดทำ** | ณัฏฐกิตติ์ รอดเรือน 68543210007-9 |
...
**ทำอะไร** สร้างคำร้องใหม่ · เซิร์ฟเวอร์จะสร้าง `id` และตั้ง `status` เป็น `pending` ให้เอง (ไม่จำเป็นต้องส่ง `id` และ `status` มา)
...
*หมายเหตุ:* `details` จะส่งเป็น array เสมอเพื่อความสม่ำเสมอในการจัดการที่ฝั่ง client

---

### 5.4 `PUT /api/requests/:id` — เปลี่ยนสถานะคำร้อง

**ทำอะไร** อัปเดตสถานะ (`status`) ของคำร้องตามรหัสที่ระบุ เช่น เมื่อเจ้าหน้าที่เริ่มรับงานหรือทำงานเสร็จสิ้น
...
*ข้อจำกัด:* ค่า `status` ต้องเป็น 1 ใน 3 ค่านี้เท่านั้น: `"pending"`, `"in-progress"`, `"completed"`
...
**ทำอะไร** ลบคำร้องออกจากระบบตามรหัสที่ระบุอย่างถาวร
...
ไม่มี body ส่งกลับ (มีเฉพาะ HTTP Status Code 204)
...
**คำถามที่พบบ่อย (FAQ)**
- **ถ้าลบแล้วยิง `GET /api/requests/:id` ซ้ำจะได้อะไร:** ตอบกลับเป็น `404 Not Found` พร้อม error message ว่าไม่พบคำร้องรหัสนี้
- **ถ้าลบซ้ำอันเดิมสองครั้งติดกัน:** ครั้งแรกจะได้ `204 No Content` ส่วนครั้งที่สองจะได้ `404 Not Found` เพราะข้อมูลถูกลบไปแล้วตั้งแต่ครั้งแรก
...
*หมายเหตุเรื่องความปลอดภัย:* ในโหมด production จะไม่ส่ง stack trace กลับไป เพื่อไม่ให้เปิดเผยโครงสร้างภายในระบบ
...
*หมายเหตุ:* CORS เป็นกลไกความปลอดภัยของเบราว์เซอร์เท่านั้น เครื่องมืออย่าง Postman หรือ `curl` จะไม่ถูกบล็อก
...
**ลำดับสำคัญ:** เปิด API ก่อนเสมอ เพื่อให้ frontend สามารถเชื่อมต่อและดึงข้อมูลเริ่มต้นได้ทันที
...
|---|---|---|:---:|
| 1.0.0 | 07/09/2569 | เวอร์ชันแรก — GET, POST, DELETE | — |
| 2.0.0 | 14/09/2569 | เพิ่ม `PUT /api/requests/:id` และ CORS | ไม่ |
...
- [x] ทุก endpoint มีตัวอย่าง request และ response จริง (ไม่ใช่เดา)
- [x] ทุก endpoint ระบุกรณีผิดพลาดครบทุกแบบ ไม่ใช่เฉพาะกรณีสำเร็จ
